...
/**
 * NotificationsDropdown — responsive notifications surface.
 *
 *  - Desktop (>= md): a classic right-aligned DropdownMenu anchored to the
 *    bell icon in the page actions.
 *  - Mobile (< md): a full-width BottomSheet that slides up from the bottom
 *    edge, with comfortable touch targets and no bottom cutoff.
 *
 * Both variants share the same content (NotificationsList) so the look &
 * feel stays consistent. The bell trigger is identical either way.
 */
export function NotificationsDropdown() {
  ...
  // ---- Shared bell trigger (used by both variants) ----
  const trigger = (
    <button
      className={cn(
...
// ============================================================
// Shared header — title + unread badge + mark-all-read + close (mobile).
// ============================================================
...
// ============================================================
// Shared list — renders grouped notification items. Used by both the
// desktop dropdown and the mobile bottom sheet so the content looks
              ...
              {items.map((n) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onAction(n)}
                    className={cn(
                      "group flex w-full items-start gap-3 border-l-2 px-4 py-3 text-left transition-colors hover:bg-muted/50 focus-visible:outline-none focus-visible:bg-muted/60 sm:py-2.5",
                      isUnread
                        ? category === "urgent"
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-muted-foreground sm:text-[11px]">
                        {n.description}
                      </p>
                      {n.action && (
                        <span className="mt-1 inline-flex items-center gap-1 text-[10px] font-medium text-teal-600 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100 dark:text-teal-400">
                          {n.action.label} →
                        </span>
                      )}
                    </div>
